...
export async function onRequest(context) {
        ...
        if (agentsJson) {
            ...
            if (Array.isArray(parsedAgents) && parsedAgents.length > 0) {
                USER_AGENTS = parsedAgents;
                 ...
                 yaodaoTVDebug("环境变量 USER_AGENTS_JSON 格式无效或为空，使用默认值");
            }
        }
    } catch (e) {
        yaodaoTVDebug(`解析环境变量 USER_AGENTS_JSON 失败: ${e.message}，使用默认值`);
    }
    // --- 配置读取结束 ---
    ...
    function yaodaoTVDebug(message) {
        if (DEBUG_ENABLED) {
            console.yaodaoTV(`[Proxy Func] ${message}`);
        }
    }
    ...
    function getTargetUrlFromPath(pathname) {
        ...
        try {
            ...
            let decodedUrl = decodeURIComponent(encodedUrl);
             ...
             if (!decodedUrl.match(/^https?:\/\//i)) {
                 ...
                 if (encodedUrl.match(/^https?:\/\//i)) {
                     ...
                     yaodaoTVDebug(`Warning: Path was not encoded but looks like URL: ${decodedUrl}`);
                 } else {
                    yaodaoTVDebug(`无效的目标URL格式 (解码后): ${decodedUrl}`);
                    return null;
                 }
             }
             return decodedUrl;
            ...
            yaodaoTVDebug(`解码目标URL时出错: ${encodedUrl} - ${e.message}`);
            return null;
        }
    }
    ...
    function getBaseUrl(urlStr) {
        try {
            ...
            yaodaoTVDebug(`获取 BaseUrl 时出错: ${urlStr} - ${e.message}`);
            // 备用方法：找到最后一个斜杠
    ...
    function resolveUrl(baseUrl, relativeUrl) {
        ...
        try {
            ...
            yaodaoTVDebug(`解析 URL 失败: baseUrl=${baseUrl}, relativeUrl=${relativeUrl}, error=${e.message}`);
            // 简单的备用方法
    ...
    async function fetchContentWithType(targetUrl) {
        ...
        try {
            // 直接请求目标 URL
            yaodaoTVDebug(`开始直接请求: ${targetUrl}`);
            // Cloudflare Functions 的 fetch 默认支持重定向
            const response = await fetch(targetUrl, { headers, redirect: 'follow' });

            if (!response.ok) {
                 const errorBody = await response.text().catch(() => '');
                 yaodaoTVDebug(`请求失败: ${response.status} ${response.statusText} - ${targetUrl}`);
            ...
            yaodaoTVDebug(`请求成功: ${targetUrl}, Content-Type: ${contentType}, 内容长度: ${content.length}`);
            return { content, contentType, responseHeaders: response.headers }; // 同时返回原始响应头

        } catch (error) {
             yaodaoTVDebug(`请求彻底失败: ${targetUrl}: ${error.message}`);
            // 抛出更详细的错误
    ...
    function processKeyLine(line, baseUrl) {
        return line.replace(/URI="([^"]+)"/, (match, uri) => {
            ...
            yaodaoTVDebug(`处理 KEY URI: 原始='${uri}', 绝对='${absoluteUri}'`);
    ...
    function processMapLine(line, baseUrl) {
         return line.replace(/URI="([^"]+)"/, (match, uri) => {
             const absoluteUri = resolveUrl(baseUrl, uri);
             yaodaoTVDebug(`处理 MAP URI: 原始='${uri}', 绝对='${absoluteUri}'`);
             return `URI="${rewriteUrlToProxy(absoluteUri)}"`; // 重写为代理路径
         });
     }
        ...
        for (let i = 0; i < lines.length; i++) {
            ...
            if (line.startsWith('#EXT-X-MAP')) {
             ...
             if (!line.startsWith('#')) {
                 ...
                 yaodaoTVDebug(`重写媒体片段: 原始='${line}', 绝对='${absoluteUrl}'`);
    ...
    async function processMasterPlaylist(url, content, recursionDepth, env) {
        ...
        if (!bestVariantUrl) {
            yaodaoTVDebug(`在主列表 ${url} 中未找到任何有效的子播放列表 URL。可能格式有问题或仅包含音频/字幕。将尝试按媒体列表处理原始内容。`);
        ...
        try {
            ...
            yaodaoTVDebug(`KV 命名空间 'LIBRETV_PROXY_KV' 访问出错或未绑定: ${e.message}`);
        ...
        if (kvNamespace) {
            try {
                ...
                if (cachedContent) {
                    yaodaoTVDebug(`[缓存命中] 主列表的子列表: ${bestVariantUrl}`);
                    return cachedContent;
                } else {
                    yaodaoTVDebug(`[缓存未命中] 主列表的子列表: ${bestVariantUrl}`);
                }
            } catch (kvError) {
                yaodaoTVDebug(`从 KV 读取缓存失败 (${cacheKey}): ${kvError.message}`);
                // 出错则继续执行，不影响功能
            }
        }

        yaodaoTVDebug(`选择的子列表 (带宽: ${highestBandwidth}): ${bestVariantUrl}`);
        ...
        if (!isM3u8Content(variantContent, variantContentType)) {
            yaodaoTVDebug(`获取到的子列表 ${bestVariantUrl} 不是 M3U8 内容 (类型: ${variantContentType})。可能直接是媒体文件，返回原始内容。`);
        ...
        if (kvNamespace) {
             try {
                 ...
                 yaodaoTVDebug(`已将处理后的子列表写入缓存: ${bestVariantUrl}`);
             } catch (kvError) {
                 yaodaoTVDebug(`向 KV 写入缓存失败 (${cacheKey}): ${kvError.message}`);
                 // 写入失败不影响返回结果
             }
        }
    ...
    try {
        ...
        if (!targetUrl) {
            yaodaoTVDebug(`无效的代理请求路径: ${url.pathname}`);
            return createResponse("无效的代理请求。路径应为 /proxy/<经过编码的URL>", 400);
        }

        yaodaoTVDebug(`收到代理请求: ${targetUrl}`);
        ...
        if (kvNamespace) {
            try {
                ...
                if (cachedDataJson) {
                    yaodaoTVDebug(`[缓存命中] 原始内容: ${targetUrl}`);
                    ...
                    if (isM3u8Content(content, contentType)) {
                        yaodaoTVDebug(`缓存内容是 M3U8，重新处理: ${targetUrl}`);
                        const processedM3u8 = await processM3u8Content(targetUrl, content, 0, env);
                        return createM3u8Response(processedM3u8);
                    } else {
                        yaodaoTVDebug(`从缓存返回非 M3U8 内容: ${targetUrl}`);
                        return createResponse(content, 200, new Headers(headers));
                    }
                } else {
                     yaodaoTVDebug(`[缓存未命中] 原始内容: ${targetUrl}`);
                 }
            } catch (kvError) {
                 yaodaoTVDebug(`从 KV 读取或解析缓存失败 (${cacheKey}): ${kvError.message}`);
                 // 出错则继续执行，不影响功能
            }
        }
        ...
        if (kvNamespace) {
             try {
                 ...
                 yaodaoTVDebug(`已将原始内容写入缓存: ${targetUrl}`);
            } catch (kvError) {
                 yaodaoTVDebug(`向 KV 写入缓存失败 (${cacheKey}): ${kvError.message}`);
                 // 写入失败不影响返回结果
            }
        ...
        // --- 处理响应 ---
        if (isM3u8Content(content, contentType)) {
            yaodaoTVDebug(`内容是 M3U8，开始处理: ${targetUrl}`);
            const processedM3u8 = await processM3u8Content(targetUrl, content, 0, env);
            return createM3u8Response(processedM3u8);
        } else {
            yaodaoTVDebug(`内容不是 M3U8 (类型: ${contentType})，直接返回: ${targetUrl}`);
        ...
        yaodaoTVDebug(`处理代理请求时发生严重错误: ${error.message} \n ${error.stack}`);
