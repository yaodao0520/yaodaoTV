...
try {
    ...
    if (agentsJsonString) {
        ...
        if (Array.isArray(parsedAgents) && parsedAgents.length > 0) {
            ...
            console.yaodaoTV(`[代理日志] 已从环境变量加载 ${USER_AGENTS.length} 个 User Agent。`);
        } else {
            console.warn("[代理日志] 环境变量 USER_AGENTS_JSON 不是有效的非空数组，使用默认值。");
        }
    } else {
        console.yaodaoTV("[代理日志] 未设置环境变量 USER_AGENTS_JSON，使用默认 User Agent。");
    }
} catch (e) {
...
function getTargetUrlFromPath(encodedPath) {
    if (!encodedPath) {
        yaodaoTVDebug("getTargetUrlFromPath 收到空路径。");
        return null;
    }
    try {
        ...
        if (decodedUrl.match(/^https?:\/\/.+/i)) {
            ...
            yaodaoTVDebug(`无效的解码 URL 格式: ${decodedUrl}`);
            // 备选检查：原始路径是否未编码但看起来像 URL？
            if (encodedPath.match(/^https?:\/\/.+/i)) {
                yaodaoTVDebug(`警告: 路径未编码但看起来像 URL: ${encodedPath}`);
                return encodedPath;
            }
            return null;
        }
        ...
        // 捕获解码错误 (例如格式错误的 URI)
        yaodaoTVDebug(`解码目标 URL 出错: ${encodedPath} - ${e.message}`);
        return null;
    }
}
...
function getBaseUrl(urlStr) {
    ...
    try {
        ...
        yaodaoTVDebug(`获取 BaseUrl 失败: "${urlStr}": ${e.message}`);
        // 备用方法：查找最后一个斜杠
...
function resolveUrl(baseUrl, relativeUrl) {
    ...
    try {
        ...
        yaodaoTVDebug(`URL 解析失败: base="${baseUrl}", relative="${relativeUrl}". 错误: ${e.message}`);
        // 简单的备用逻辑
...
async function fetchContentWithType(targetUrl, requestHeaders) {
    ...
    yaodaoTVDebug(`准备请求目标: ${targetUrl}，请求头: ${JSON.stringify(headers)}`);
    ...
    try {
        ...
        yaodaoTVDebug(`请求异常 ${targetUrl}: ${error.message}`);
        // 重新抛出，确保包含原始错误信息
...
function processMediaPlaylist(url, content) {
    ...
    if (!baseUrl) {
        yaodaoTVDebug(`无法确定媒体列表的 Base URL: ${url}，相对路径可能无法处理。`);
    }
    ...
    for (let i = 0; i < lines.length; i++) {
        ...
        if (!line.startsWith('#')) {
            ...
            yaodaoTVDebug(`重写媒体片段: 原始='${line}', 解析后='${absoluteUrl}'`);
...
async function processM3u8Content(targetUrl, content, recursionDepth = 0) {
    ...
    if (content.includes('#EXT-X-STREAM-INF') || content.includes('#EXT-X-MEDIA:')) {
        yaodaoTVDebug(`检测到主播放列表: ${targetUrl} (深度: ${recursionDepth})`);
        return await processMasterPlaylist(targetUrl, content, recursionDepth);
    }
    yaodaoTVDebug(`检测到媒体播放列表: ${targetUrl} (深度: ${recursionDepth})`);
...
async function processMasterPlaylist(url, content, recursionDepth) {
    ...
    if (!bestVariantUrl) {
        yaodaoTVDebug(`主播放列表中未找到 BANDWIDTH 信息，尝试查找第一个 URI: ${url}`);
        for (let i = 0; i < lines.length; i++) {
            const line = lines[i].trim();
             // 更可靠地匹配 .m3u8 链接
            if (line && !line.startsWith('#') && line.match(/\.m3u8($|\?.*)/i)) {
                bestVariantUrl = resolveUrl(baseUrl, line);
                yaodaoTVDebug(`备选方案: 找到第一个子播放列表 URI: ${bestVariantUrl}`);
                break;
            }
        }
    }
    // 如果仍然没有找到子列表 URL
    if (!bestVariantUrl) {
        yaodaoTVDebug(`在主播放列表 ${url} 中未找到有效的子列表 URI，将其作为媒体列表处理。`);
        return processMediaPlaylist(url, content);
    }

    yaodaoTVDebug(`选择的子播放列表 (带宽: ${highestBandwidth}): ${bestVariantUrl}`);
    ...
    if (!isM3u8Content(variantContent, variantContentType)) {
        yaodaoTVDebug(`获取的子播放列表 ${bestVariantUrl} 不是 M3U8 (类型: ${variantContentType})，将其作为媒体列表处理。`);
